package body Aida.UTF8 with SPARK_Mode is
   ...
   begin
      Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer)));
      ...
      case Code is
         ...
         when 16#C2#..16#DF# => -- 2 bytes
            ...
            Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer + 1)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 2;
         when 16#E0# => -- 3 bytes
            Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer + 1)));
            Accum := (Code and 16#3F#) * 2**6;
            Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer + 2)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 3;
         ...
         when 16#F0# => -- 4 bytes
            ...
            Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer + 2)));
            Accum := Accum or (Code and 16#3F#) * 2**6;
            Code := Aida.UTF8_Code_Point.T (Character_T'Pos (Source (Pointer + 3)));
            Value   := Accum or (Code and 16#3F#);
            Pointer := Pointer + 4;
   ...
   begin
      if Value <= 16#7F# then
         Destination (Pointer) := Character_T'Val (Value);
         Pointer := Pointer + 1;
      elsif Value <= 16#7FF# then
         Destination (Pointer) := Character_T'Val (16#C0# or Value / 2**6);
         Destination (Pointer + 1) := Character_T'Val (16#80# or (Value and 16#3F#));
         Pointer := Pointer + 2;
      elsif Value <= 16#FFFF# then
         Destination (Pointer) := Character_T'Val (16#E0# or Value / 2**12);
         Destination (Pointer + 1) := Character_T'Val (16#80# or (Value / 2**6 and 16#3F#));
         Destination (Pointer + 2) := Character_T'Val (16#80# or (Value and 16#3F#));
         Pointer := Pointer + 3;
      else
         Destination (Pointer) := Character_T'Val (16#F0# or Value / 2**18);
         Destination (Pointer + 1) := Character_T'Val (16#80# or (Value / 2**12 and 16#3F#));
         Destination (Pointer + 2) := Character_T'Val (16#80# or (Value / 2**6 and 16#3F#));
         Destination (Pointer + 3) := Character_T'Val (16#80# or (Value and 16#3F#));
         Pointer := Pointer + 4;
      end if;
   end Put;
