...
is
   ...
   type State_Id_Type is
     (
      Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket,
      --  NL = New Line
      ...
      begin
         while P <= Contents'Last loop
            ...
            pragma Loop_Invariant
              (((Prev_Prev_P = Contents'First and Prev_P = Contents'First)
               and then P > Contents'First) or
                 ((Prev_Prev_P = Contents'First and Prev_P > Contents'First)
                  and then Prev_P < P) or
                 (Prev_Prev_P > Contents'First
                  and then (Prev_Prev_P < Prev_P and Prev_P < P)));

            pragma Loop_Invariant
              (State_Id /= Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket or
                 (State_Id = Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket
                  and then (Tag_Ids.Is_Empty)));
            pragma Loop_Invariant
              (State_Id /= Extracting_Key_Name or
                 (State_Id = Extracting_Key_Name and then
                      (Key_Name_First_Index <= Contents'Last and
                           Tag_Ids.Last_Index >= Tag_Ids.First_Index)));
            pragma Loop_Invariant
              (State_Id /= Found_End_Of_The_Very_Last_Object or
                 (State_Id = Found_End_Of_The_Very_Last_Object and then
                      (Tag_Ids.Is_Empty)));
            pragma Loop_Invariant
              ((State_Id = Extracting_Key_Name or
                   State_Id = Found_End_Of_The_Very_Last_Object
               or State_Id = Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket)
               or ((State_Id /= Extracting_Key_Name and
                   State_Id /= Found_End_Of_The_Very_Last_Object
              and State_Id /= Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket)
                 and then (Tag_Ids.Last_Index >= Tag_Ids.First_Index)));
            ...
            case State_Id is
               when Expecting_NL_Sign_Or_Space_Or_Left_Curly_Bracket =>
                  if CP = Character'Pos ('{') then
                     ...
                     if not Tag_Ids.Is_Full and
                       Next_Tag_Id < Tag_Id_T'Last
                     then
